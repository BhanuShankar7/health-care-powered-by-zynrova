        ...
        <div className="symptoms-page">
            ...
            <section className="sym-highlight-section">
                <div className="sym-detail-card">
                    ...
                    <div style={{ fontSize: '15px', color: '#27B992', fontWeight: '700', marginBottom: '20px' }}>Recommended Departments</div>
                        ...
                        {symptomsData[activeSymptom].departments.map((dept, idx) => (
                            ...
                            >
                                <div style={{ display: 'flex', alignItems: 'center', gap: '15px', flex: 1 }}>
                                    <div>
                                        <div style={{ fontSize: '18px', fontWeight: '800', color: '#333', marginBottom: '4px' }}>{dept.name}</div>
                                        <div style={{ fontSize: '14px', color: '#999', fontWeight: '400' }}>{dept.sub}</div>
                                    </div>
                                </div>
                                <div style={{ color: '#27B992', background: '#eaf8f5', borderRadius: '50%', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div className="body-menu">
                        {bodyAreas.map((area) => (
                            <div
                                ...
                                onClick={() => {
                                    setActiveBodyArea(area.name);
                                    let generalTarget = 'General Physician';
                                    if (area.name === 'Joints & Bones') generalTarget = 'Orthopedic';
                                    else if (area.name === 'Chest & Respiratory') generalTarget = 'Cardiologist';
                                    
                                    navigate('/find-doctors', { state: { specialty: generalTarget } });
                                }}
                            >
                    ...
                    <div className="body-detail-panel">
                        ...
                        <div className="symptom-list-grid">
                            {bodySymptoms[activeBodyArea].map((sym, i) => {
                                // Simple mapping for demonstration based on the dummy text
                                let targetSpecialty = 'General Physician';
                                if (['Arthritis', 'Back Pain', 'Knee Pain', 'Neck Stiffness', 'Muscle Cramp'].includes(sym)) targetSpecialty = 'Orthopedic';
                                else if (['Chest Pain'].includes(sym)) targetSpecialty = 'Cardiologist';
                                
                                return (
                                    <div 
                                        className="symptom-item" 
                                        key={i}
                                        onClick={() => navigate('/find-doctors', { state: { specialty: targetSpecialty } })}
                                    >
                                        {sym}
